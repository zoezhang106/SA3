...
if (aortic_valve_stenosis == 1) {
    R_art  = R_art0*(1+R_art_stenosis_factor*(1-exp(-stenosis_rate*sim_time)));  #make sure to set init sim_time = 0
...
if (mitral_regurgitation == 1) {
  #decrease threshold for leak gradually over time
  mitral_regurgitation_pressure_diff = min_mitral_diff + max_mitral_diff*(exp(-k_mitral_diff*sim_time));
} else {
...
rsna_HR_intercept = 1-rsna_HR_slope;

rsna_effect_on_HR =  rsna_HR_slope * renal_sympathetic_nerve_activity + rsna_HR_intercept;

heart_rate = HR_heart_rate *rsna_effect_on_HR*beta_blocker_effect_on_heart_rate;
...
mean_venous_pressure = (systolic_venous_pressure/3+diastolic_venous_pressure*2/3);

########### Vascular Autoregulation #############

#Local tissue autoregulation
tissue_autoregulation_sig1 = -tissue_autoreg_scale*(Kp_CO*(CO_delayed - CO_nom*CO_species_scale)+Ki_CO*CO_error);
tissue_autoregulation_signal = 1/((0.2*tissue_autoregulation_sig1 + 1)^4);
...
venous_autoregulation_signal_int = 1-venous_autoregulation_signal_scale;
venous_autoregulation_signal = venous_autoregulation_signal_int + venous_autoregulation_signal_scale/(1+exp(((tissue_autoregulation_sig1 - venous_autoregulation_signal_Km)-1)/venous_autoregulation_signal_slope));
V_ven0_adjusted = V_ven0*venous_autoregulation_signal;

########### Volume unit conversions ########### 
LV_volume_mL = LV_volume * m3_mL;
arterial_volume_mL = arterial_volume * m3_mL;
peripheral_volume_mL = peripheral_circulation_volume * m3_mL;
...
baseline_total_myocyte_volume = V_w_0 - 
                                Baseline_Interstitial_Fibrosis - 
                                Baseline_Replacement_Fibrosis - 
                                Baseline_Interstitial_Tissue;  ## baseline myocyte volume determined by V_w_0
...
myocyte_length = Baseline_Myocyte_Length + 
                change_in_myocyte_length;		## change_in_myocyte_length depends on passive stress levels 

myocyte_diameter = baseline_myocyte_diameter + 
                change_in_myocyte_diameter;	## change_in_myocyte_diameter depends on active stress levels - HYPERTROPHY
...
total_nonmyocyte_volume = Baseline_Interstitial_Fibrosis + 
                          Baseline_Interstitial_Tissue + 
                          Baseline_Replacement_Fibrosis;

LV_wall_volume = total_myocyte_volume + 
                  total_nonmyocyte_volume;
...
pct_change_in_myocyte_diameter = 100 * (change_in_myocyte_diameter / baseline_myocyte_diameter);
pct_change_in_myocyte_length = 100 * (change_in_myocyte_length / Baseline_Myocyte_Length ) ;
...
LV_cavity_volume = LV_V0_baseline * 
                  ((1 + myo_L_scale * change_in_myocyte_length / Baseline_Myocyte_Length) ^ 3) * 
                   ((1 - myo_D_scale * change_in_myocyte_diameter / baseline_myocyte_diameter) ^ 2);
...
contraction_velocity_effect_in_LV = (1 - LV_sarcomere_contraction_velocity / v0_LV_contraction_velocity_effect_in_LV) / 
                                    (1 + Cv_contraction_velocity_effect_in_LV * LV_sarcomere_contraction_velocity / v0_LV_contraction_velocity_effect_in_LV);

if (LV_sarcomere_length > ls_a0) {
    sarcomere_length_effect_in_LV = ((LV_sarcomere_length - ls_a0) / 
                                    (ls_ar_sarcomere_length_effect_in_LV - ls_a0));
} else {
...
t_d = tau_d_LV_twitch_shape*(1+BB_cardiac_relax_effect*BB_signal); 
...
LV_active_stress = contractility * 
                    LV_twitch_shape *
                    sigma_ar * 
                    sarcomere_length_effect_in_LV *  
                    contraction_velocity_effect_in_LV * 
                    (beta_blocker_effect_on_contractility * sna_effect_on_contractility);

#Increase in left ventricle stiffness with increasing hypertrophy
hypertrophy_effect_on_Cf = hypertrophy_Cf_slope*max(0,(level_of_hypertrophy - 1));

C_f = cf*(1+hypertrophy_effect_on_Cf);

#Unpressurized fiber stretch
stretch_zero_S = stretch_min_LV_passive_stress_along_fiber - 
                  stretch_scale_LV_passive_stress_along_fiber;
                  
if (LV_fiber_stretch >= stretch_zero_S) {
  LV_passive_stress_along_fiber = s_f0 * 
                                  (exp(C_f * (LV_fiber_stretch - stretch_zero_S)) - 1);
} else {
...
if (LV_radial_stretch >= 1) {
  LV_passive_radial_stress = s_r0 * 
                              (exp(c_r_LV * (LV_radial_stretch - 1)) - 1);
} else {
  LV_passive_radial_stress = 0;
}

#Total stress is sum of active and passive stresses
LV_total_stress = LV_active_stress + 
                  LV_passive_stress_along_fiber - 
                  2 * LV_passive_radial_stress;
...
RV_sarcomere_contraction_velocity = (RV_sarcomere_length - RV_sarcomere_length_delayed) / 
                                    time_step;

contraction_velocity_effect_in_RV = (1 - RV_sarcomere_contraction_velocity / v0_RV_contraction_velocity_effect_in_RV) / 
                                    (1 + 0 * RV_sarcomere_contraction_velocity / v0_RV_contraction_velocity_effect_in_RV);

RV_active_stress_multiplier = contractility_RV*sna_effect_on_contractility;

RV_active_stress = contractility_RV * 
                    RV_twitch_shape * 
                    sigma_ar_RV * 
                    sarcomere_length_effect_in_RV * 
                    contraction_velocity_effect_in_RV*
...
RV_total_stress = RV_active_stress + 
                  RV_passive_stress_along_fiber - 
                  2 * RV_passive_radial_stress;
...
peripheral_pressure = P_ven0 + 
                      (peripheral_circulation_volume - V_per0) / C_per;

venous_compliance=C_ven0*
                  AngII_effect_on_venous_compliance*
                  (SNA_effect_on_venous_compliance*BB_venous_effect);  

venous_pressure = P_ven0 + 
                  (venous_volume - V_ven0) / venous_compliance;

venous_flow = (peripheral_pressure - venous_pressure) / R_ven0;


##allow blood volume link between heart and kidney to be turned on/off
if (heart_renal_link == 1) {
	venous_volume_target =  blood_volume - LV_volume - arterial_volume - peripheral_circulation_volume - RV_volume - pulmonary_arterial_volume - pulmonary_venous_volume;
} else {
...
pulmonary_arterial_pressure = ( pulmonary_arterial_volume - V_pulm_art0 )/(C_pulm_art) + 
                              P_art0;

pulmonary_venous_pressure = P_ven0 + 
                            (pulmonary_venous_volume  - V_pulm_ven0 )/(C_pulm_ven*BB_venous_effect);

pulmonary_arterial_blood_flow = (pulmonary_arterial_pressure  - pulmonary_venous_pressure )/ R_ven_pulm ;

dP = RV_pressure - 
      pulmonary_arterial_pressure;

Zn = L_pulm + 
      time_step * R_art_pulm;
...
if (mitral_valve_flow_rate < 0) {
    mitral_valve_leak_rate = mitral_valve_flow_rate;
} else {
  mitral_valve_leak_rate = 0;
}
...
peripheral_resistance_multiplier = disease_effect_on_TPR_peripheral_resistance * 
                                    B2sna_effect_on_TPR*
                                    A1sna_effect_on_TPR*(1-(1-BB_TPR_effect)*BB_signal)*
                                    tissue_autoregulation_signal;

peripheral_resistance_multiplier_adjusted = 1+vascular_responsiveness_scale*(peripheral_resistance_multiplier-1);

peripheral_resistance = R_per0* TPR_scale_peripheral_resistance * 
                        peripheral_resistance_multiplier_adjusted;
...
C_art  = (C_art_initial-C_art_initial*C_art_scale)/
        exp(sim_time/(24/2))+C_art_initial*C_art_scale;

Stiffness0=1/C_art;

arterial_stiffness = Stiffness0*
                    (1+ (mean_arterial_pressure_MAP - nominal_map_setpoint)*Stiffness_BP_slope);
...
systemic_blood_flow = (arterial_pressure - peripheral_pressure) / peripheral_resistance;


dP_1 = LV_pressure - arterial_pressure;

Zn_1 = L_art + R_art * time_step;

aortic_blood_flow = (aortic_blood_flow_delayed * L_art + dP_1 * time_step) / Zn_1;
...
if (heart_renal_link  == 1) {
  if (ANP_infusion == 1) {
    ANP = ANP_infused_concentration;
  }  else {
    ANP = nom_ANP*LVEDP_ANP_effect; #pg/ml
  }
  normalized_ANP = ANP/nom_ANP;
...
if (venous_pressure_delayed < venous_pressure_bigger_delay) {
  systemic_venous_pressure_minimum_1 = venous_pressure_delayed;
} else {
  systemic_venous_pressure_minimum_1 = diastolic_venous_pressure;
}

if (venous_pressure_delayed < venous_pressure) {
  systemic_venous_pressure_minimum = systemic_venous_pressure_minimum_1;
} else {
  systemic_venous_pressure_minimum = diastolic_venous_pressure;
}

if (venous_pressure_delayed > venous_pressure_bigger_delay) {
  systemic_venous_pressure_maximum_1 = venous_pressure_delayed;
} else {
  systemic_venous_pressure_maximum_1 = systolic_venous_pressure;
}

if (venous_pressure_delayed > venous_pressure) {
  systemic_venous_pressure_maximum= systemic_venous_pressure_maximum_1;
} else {
  systemic_venous_pressure_maximum= systolic_venous_pressure;
}

systolic_venous_pressure_old = systemic_venous_pressure_maximum;

diastolic_venous_pressure_old = systemic_venous_pressure_minimum;
...
if (LV_active_stress_peak > LV_active_stress_threshhold) {		
  ...
  kD_hypertrophy =  (kD_HYPERTROPHY*C_renal_CV_timescale) * 
                    max(0, ((max_myocyte_diameter_increase) - change_in_myocyte_diameter)/
                            (max_myocyte_diameter_increase));	
} else {	
...
number_of_functional_glomeruli = baseline_nephrons;
...
creatinine_clearance_rate = GFR_ml_min * dl_ml * 
                            serum_creatinine_concentration; #Units: mg/min
...
Na_concentration = sodium_amount / 
                    blood_volume_L;
IF_Na_concentration = IF_sodium_amount/
                      interstitial_fluid_volume;
...
SN_filtered_Na_load = (SNGFR_nL_min / 1000 / 1000000)*
                      Na_concentration;

filtered_Na_load = SN_filtered_Na_load*
...
aldosterone_concentration = normalized_aldosterone_level* 
                            nominal_aldosterone_concentration; 

Aldo_MR_normalised_effect = normalized_aldosterone_level*
                              (1 - pct_target_inhibition_MRA);

aldo_DCT_int = 1 - aldo_DCT_scale/2;

aldo_effect_on_DCT = aldo_DCT_int + aldo_DCT_scale/
                          (1+exp((1 - Aldo_MR_normalised_effect)/aldo_DCT_slope));

aldo_CD_int = 1 - aldo_CD_scale/2;

aldo_effect_on_CD= aldo_CD_int + aldo_CD_scale/
                      (1+exp((1 - Aldo_MR_normalised_effect)/aldo_CD_slope));
...
glucose_pt_out_s1 = max(0,SN_filtered_glucose_load-
                            glucose_reabs_per_unit_length_s1*L_pt_s1); #mmol/min
...
SGTL2_Na_reabs_mmol_s1 = SN_filtered_glucose_load-
                          glucose_pt_out_s1;   		#mmol/min

SGTL2_Na_reabs_mmol_s2 = glucose_pt_out_s1-
                          glucose_pt_out_s2;			#mmol/min

SGTL1_Na_reabs_mmol = 2*(glucose_pt_out_s2-glucose_pt_out_s3);			#mmol/min

total_SGLT_Na_reabs = SGTL2_Na_reabs_mmol_s1 +
                      SGTL2_Na_reabs_mmol_s2 +
                      SGTL1_Na_reabs_mmol; 	#mmol/min

Na_reabs_per_unit_length = -log(1-e_pt_sodreab)/
                            (L_pt_s1+L_pt_s2+L_pt_s3); #non-SGLT2 reabs	#mmol/min

Na_pt_s1_reabs = min(max_s1_Na_reabs, 
                    SN_filtered_Na_load*
                    (1-exp(-Na_reabs_per_unit_length*L_pt_s1)));
                    
Na_pt_out_s1 = SN_filtered_Na_load - 
                Na_pt_s1_reabs - 
                SGTL2_Na_reabs_mmol_s1 ;

Na_pt_s2_reabs = min(max_s2_Na_reabs, 
                    Na_pt_out_s1*
                    (1-exp(-Na_reabs_per_unit_length*L_pt_s2)));
                    
Na_pt_out_s2 = Na_pt_out_s1 - 
              Na_pt_s2_reabs - 
              SGTL2_Na_reabs_mmol_s2;

Na_pt_s3_reabs = min(max_s3_Na_reabs, 
                  Na_pt_out_s2*
                  (1-exp(-Na_reabs_per_unit_length*L_pt_s3)));

Na_pt_out_s3 = Na_pt_out_s2 - 
                Na_pt_s3_reabs - 
                SGTL1_Na_reabs_mmol;

PT_Na_reabs_fraction = 1-Na_pt_out_s3/
                        SN_filtered_Na_load;
...
urea_out_s1 = SN_filtered_urea_load - 
              ...
              water_out_s1_delayed; #For now, assuming only reabsorbed at the end

urea_out_s2 = urea_out_s1 - 
              urea_permeability_PT*
              (urea_out_s1/(0.5*(water_out_s1_delayed+water_out_s2_delayed))-plasma_urea)*
              water_out_s2_delayed; #For now, assuming only reabsorbed at the end

urea_out_s3 = urea_out_s2 - 
              urea_permeability_PT*
              (urea_out_s2/(0.5*(water_out_s2_delayed+water_out_s3_delayed))-plasma_urea)*
              water_out_s3_delayed; #For now, assuming only reabsorbed at the end
...
osmoles_out_s2 = 2*Na_pt_out_s2 + 
                 glucose_pt_out_s2 +
                 urea_out_s2;
                 
water_out_s2 = (water_out_s1/osmoles_out_s1)*
                osmoles_out_s2;

osmoles_out_s3 = 2*Na_pt_out_s3 + 
                  glucose_pt_out_s3 + 
                  urea_out_s3;
                  
water_out_s3 = (water_out_s2/osmoles_out_s2)*
                osmoles_out_s3;

PT_water_reabs_fraction = 1-water_out_s3/
                          (SNGFR_nL_min / 1000 / 1000000);
...
PT_Na_reab_perUnitSA = SN_filtered_Na_load*e_pt_sodreab/
...
## =================================================================
## creation of intermediate variables for debugging purposes -- to be deleted
## =================================================================
ddt_bowman = C_renal_CV_timescale*100*(P_in_pt_s1_mmHg - P_bowmans);


## time derivatives actual useful code
d/dt(venous_volume) = venous_flow  - tricuspid_valve_flow_rate + C_renal_CV_timescale*(venous_volume_target - venous_volume);
...
d/dt(systolic_venous_pressure) = C_cycle2 * (systolic_venous_pressure_old - systolic_venous_pressure);

d/dt(diastolic_venous_pressure) = C_cycle2 * (diastolic_venous_pressure_old - diastolic_venous_pressure);
...
d/dt(UGE) = C_renal_CV_timescale**RUGE; #mg/hr
...
d/dt(mitral_valve_leak) = mitral_valve_leak_rate;
"
